...
var parseTranslations = function(originalStrings, done) {
  return function(err, data) {
    ...
    data = data.map(function(translation){
      translation.translatedText = entities.decode(translation.translatedText).replace(/% ([a-zA-Z0-9])/g, (match, p1) => `%${p1}`);
      return translation;
    });
...
module.exports = function(apiKey, newConcurrentLimit) {
  ...
  api.translate = function(strings, sourceLang, targetLang) {
    return new Promise((resolve, reject) => {
      if (typeof strings !== 'string' && !Array.isArray(strings)) reject('Input source must be a string or array of strings');
      if (typeof sourceLang !== 'string') reject('No target language specified. Must be a string');
